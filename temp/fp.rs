...
trait Applicative<U>: Functor<U> {
    ...
    where
        Self: HKT<U, C = U>;
    fn seq<F>(&self, <Self as HKT<F>>::T) -> Self::T
    where
        F: Fn(&Self::C) -> U;
}

impl<T, U> Applicative<U> for Option<T> {
    fn pure_(value: U) -> Option<U> {
        Some(value)
    }

    fn seq<F>(&self, fs: Option<F>) -> Option<U>
    where
        F: Fn(&T) -> U,
    {
        match *self {
...
impl<T, U> Applicative<U> for Vec<T> {
    fn pure_(value: U) -> Vec<U> {
        vec![value]
    }

    fn seq<F>(&self, fs: Vec<F>) -> Vec<U>
    where
        F: Fn(&T) -> U,
    {
        let mut result = vec![];
...
impl<T, U> Applicative<U> for Rc<T> {
    fn pure_(value: U) -> Rc<U> {
        Rc::new(value)
    }

    fn seq<F>(&self, fs: Rc<F>) -> Rc<U>
    where
        F: Fn(&T) -> U,
    {
        let v = fs(self);
...
impl<T, U> Applicative<U> for Box<T> {
    fn pure_(value: U) -> Box<U> {
        Box::new(value)
    }

    fn seq<F>(&self, fs: Box<F>) -> Box<U>
    where
        F: Fn(&T) -> U,
    {
        let v = fs(self);
...
impl<T, U> Monad<U> for Option<T> {
    fn bind<F>(&self, mut f: F) -> Option<U>
    where
        F: FnMut(&T) -> Option<U>,
    {
        match *self {
